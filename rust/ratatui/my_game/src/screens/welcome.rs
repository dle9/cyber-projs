...
impl App {
    pub fn render_welcome_screen(&self, frame: &mut Frame, area: Rect) {
        // partition the frame
        ...
        let title = Line::from(Span::styled(
            format!("Welcome, {}", self.player.name),
        ...
        let msg = vec![
            ...
            Line::from(vec![Span::from("")]),
            Line::from(vec![
                Span::styled("Welcome", Style::default().add_modifier(Modifier::ITALIC)),
            ]),
        ];
